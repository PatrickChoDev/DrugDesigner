...
class Attention_new(nn.Module):
    def __init__(self, dim, heads, attention_dropout=0.0):
        super().__init__()
        assert dim % heads == 0
        self.heads = heads
...
class Encoder_Block(nn.Module):
    def __init__(self, dim, heads, act, mlp_ratio=4, drop_rate=0.0):
        super().__init__()
...
class TransformerEncoder(nn.Module):
    def __init__(self, dim, depth, heads, act, mlp_ratio=4, drop_rate=0.1):
        ...
        self.Encoder_Blocks = nn.ModuleList(
            [Encoder_Block(dim, heads, act, mlp_ratio, drop_rate) for i in range(depth)]
        )
...
class enc_dec_attention(nn.Module):
    def __init__(self, dim, heads, attention_dropout=0.0, proj_dropout=0.0):
        super().__init__()
        self.dim = dim
        self.heads = heads
        self.scale = 1.0 / dim**0.5
...
class Decoder_Block(nn.Module):
    ...
    def forward(self, mol_annot, prot_annot, mol_adj, prot_adj):
        ...
        mx1, prot_annot, ma1, prot_adj = self.dec_attn(
            mol_annot, prot_annot, mol_adj, prot_adj
        )
...
class TransformerDecoder(nn.Module):
    def __init__(self, dim, depth, heads, mlp_ratio=4, drop_rate=0.0):
        super().__init__()

        self.Decoder_Blocks = nn.ModuleList(
            [Decoder_Block(dim, heads, mlp_ratio, drop_rate) for i in range(depth)]
        )

    def forward(self, mol_annot, prot_annot, mol_adj, prot_adj):

        for Decoder_Block in self.Decoder_Blocks:
            mol_annot, prot_annot, mol_adj, prot_adj = Decoder_Block(
                mol_annot, prot_annot, mol_adj, prot_adj
            )
