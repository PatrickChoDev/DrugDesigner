...
class Solver(object):
    ...
    def train(self):
        ...
        self.arguments = "glr{}_dlr{}_g2lr{}_d2lr{}_dim{}_depth{}_heads{}_decdepth{}_decheads{}_ncritic{}_batch{}_epoch{}_warmup{}_dataset{}_disc-{}_la{}_dropout{}".format(
            self.g_lr,
            self.d_lr,
            self.g2_lr,
            self.d2_lr,
        ...
        for idx in range(self.num_iters):
            ...
            for i, data in enumerate(self.loader):
                ...
                if (i + 1) % self.n_critic == 0:
                    ...
                    g_edges_logits_forGAN2, g_nodes_logits_forGAN2 = (
                        g_edges_logits.detach().clone(),
                        g_nodes_logits.detach().clone(),
                    )
                ...
                if (idx + 1 > self.warm_up_steps) & (i + 1 > 10):
                    ...
                    tra_edges_logits, tra_nodes_logits, dec_attn = self.G2(
                        g_edges_logits_forGAN2, g_nodes_logits_forGAN2, prot_n, prot_e
                    )
                ...
                if (i + 1) % (self.log_step) == 0:
                    ...
                    if idx + 1 > self.warm_up_steps:
                        log2 = "Elapsed [{}] , Epoch/Iteration [{}/{}] for GAN2".format(
                            et, idx + 1, i + 1
                        )
                        m2, m3 = all_scores(
                            fake_mol2, drugs_mol, self.vertexes, norm=True
                        )
                        m2 = {
                            k: np.array(v)[np.nonzero(v)].mean() for k, v in m2.items()
                        }
                        m2.update(m3)
                        loss2.update(m2)
