...
class Trainer(object):
    """Trainer for training and testing DrugDesigner."""
    ...
    def __init__(self, config):
        ...
        self.raw_file = (
            config.raw_file
        )  # SMILES containing text file for first dataset.
        # Write the full path to file.

        self.drug_raw_file = (
            config.drug_raw_file
        )  # SMILES containing text file for second dataset.
        # Write the full path to file.
        ...
        self.drugs_dataset_file = (
            config.drug_dataset_file
        )  # Drug dataset file name for the second GAN.
        ...
        self.inf_drug_raw_file = (
            config.inf_drug_raw_file
        )  # SMILES containing text file for second dataset.
        # Write the full path to file.

        self.inf_dataset_file = (
            config.inf_dataset_file
        )  # Dataset file name for the first GAN.
        # Contains large number of molecules.

        self.inf_drugs_dataset_file = (
            config.inf_drug_dataset_file
        )  # Drug dataset file name for the second GAN.
        # Contains drug molecules only. (In this case AKT1 inhibitors.)
        ...
        self.drug_data_dir = (
            config.drug_data_dir
        )  # Directory where the drug dataset files are stored.
        ...
        self.features = (
            config.features
        )  # Small model uses atom types as node features. (Boolean, False uses atom types only.)
        ...
        self.dataset = DruggenDataset(
            self.mol_data_dir,
            self.dataset_file,
            self.raw_file,
            self.max_atom,
            self.features,
        )  # Dataset for the first GAN. Custom dataset class from PyG parent class.
        ...
        self.loader = DataLoader(
            self.dataset, shuffle=True, batch_size=self.batch_size, drop_last=True
        )  # PyG dataloader for the first GAN.

        self.drugs = DruggenDataset(
            self.drug_data_dir,
            self.drugs_dataset_file,
            self.drug_raw_file,
            self.max_atom,
            self.features,
        )  # Dataset for the second GAN. Custom dataset class from PyG parent class.
        # Can create any molecular graph dataset given smiles string.
        # Nonisomeric SMILES are suggested but not necessary.
        # Uses sparse matrix representation for graphs,
        # For computational and speed efficiency.
        ...
        self.drugs_loader = DataLoader(
            self.drugs, shuffle=True, batch_size=self.batch_size, drop_last=True
        )  # PyG dataloader for the second GAN.

        # Atom and bond type dimensions for the construction of the model.

        self.atom_decoders = self.decoder_load(
            "atom"
        )  # Atom type decoders for first GAN.
        # eg. 0:0, 1:6 (C), 2:7 (N), 3:8 (O), 4:9 (F)

        self.bond_decoders = self.decoder_load(
            "bond"
        )  # Bond type decoders for first GAN.
        # eg. 0: (no-bond), 1: (single), 2: (double), 3: (triple), 4: (aromatic)

        self.m_dim = (
            len(self.atom_decoders)
            if not self.features
            else int(self.loader.dataset[0].x.shape[1])
        )  # Atom type dimension.

        self.b_dim = len(self.bond_decoders)  # Bond type dimension.

        self.vertexes = int(
            self.loader.dataset[0].x.shape[0]
        )  # Number of nodes in the graph.

        self.drugs_atom_decoders = self.drug_decoder_load(
            "atom"
        )  # Atom type decoders for second GAN.
        # eg. 0:0, 1:6 (C), 2:7 (N), 3:8 (O), 4:9 (F)

        self.drugs_bond_decoders = self.drug_decoder_load(
            "bond"
        )  # Bond type decoders for second GAN.
        # eg. 0: (no-bond), 1: (single), 2: (double), 3: (triple), 4: (aromatic)

        self.drugs_m_dim = (
            len(self.drugs_atom_decoders)
            if not self.features
            else int(self.drugs_loader.dataset[0].x.shape[1])
        )  # Atom type dimension.

        self.drugs_b_dim = len(self.drugs_bond_decoders)  # Bond type dimension.

        self.drug_vertexes = int(
            self.drugs_loader.dataset[0].x.shape[0]
        )  # Number of nodes in the graph.
    ...
    def build_model(self):
        ...
        self.D2 = simple_disc(
            "tanh", self.drugs_m_dim, self.drug_vertexes, self.drugs_b_dim
        )
    ...
    def restore_model(self, epoch, iteration, model_directory):
        ...
        print(
            "Loading the trained models from epoch / iteration {}-{}...".format(
                epoch, iteration
            )
        )
        ...
        G2_path = os.path.join(
            model_directory, "{}-{}-G2.ckpt".format(epoch, iteration)
        )
        D2_path = os.path.join(
            model_directory, "{}-{}-D2.ckpt".format(epoch, iteration)
        )
    ...
    def save_model(self, model_directory, idx, i):
        ...
        if self.submodel != "NoTarget" and self.submodel != "CrossLoss":
            G2_path = os.path.join(
                model_directory, "{}-{}-G2.ckpt".format(idx + 1, i + 1)
            )
            D2_path = os.path.join(
                model_directory, "{}-{}-D2.ckpt".format(idx + 1, i + 1)
            )
    ...
    def train(self):
        ...
        # wandb.init(project="DrugDesigner2", entity="atabeyunlu")

        # Defining sampling paths and creating logger

        self.arguments = "{}_glr{}_dlr{}_g2lr{}_d2lr{}_dim{}_depth{}_heads{}_decdepth{}_decheads{}_ncritic{}_batch{}_epoch{}_warmup{}_dataset{}_dropout{}".format(
            self.submodel,
            self.g_lr,
            self.d_lr,
            self.g2_lr,
        ...
        full_smiles = [
            line
            for line in open("DrugDesigner/data/chembl_train.smi", "r")
            .read()
            .splitlines()
        ]
        drug_smiles = [
            line
            for line in open("DrugDesigner/data/akt_train.smi", "r").read().splitlines()
        ]
        ...
        akt1_human_adj = (
            torch.load("DrugDesigner/data/akt/AKT1_human_adj.pt")
            .reshape(1, -1)
            .to(self.device)
            .float()
        )
        akt1_human_annot = (
            torch.load("DrugDesigner/data/akt/AKT1_human_annot.pt")
            .reshape(1, -1)
            .to(self.device)
            .float()
        )
        ...
        if self.resume:
            self.restore_model(
                self.resume_epoch, self.resume_iter, self.resume_directory
            )
        ...
        for idx in range(self.epoch):
            ...
            for i, data in enumerate(self.loader):
                ...
                g_loss, fake_mol, g_edges_hat_sample, g_nodes_hat_sample, node, edge = (
                    generator_output
                )
                ...
                if self.submodel != "NoTarget" and self.submodel != "CrossLoss":
                    ...
                    (
                        g2_loss,
                        fake_mol_g,
                        dr_g_edges_hat_sample,
                        dr_g_nodes_hat_sample,
                    ) = output
                ...
                if (i + 1) % self.log_step == 0:
                    ...
                    if self.submodel != "NoTarget" and self.submodel != "CrossLoss":
                        ...
                        mol_sample(
                            ...
                            "GAN2",
                            fake_mol_g,
                            dr_g_edges_hat_sample.detach(),
                            dr_g_nodes_hat_sample.detach(),
                            idx,
                            i,
                        )
    ...
    def inference(self):
        ...
        if self.submodel != "NoTarget" and self.submodel != "CrossLoss":
            G2_path = os.path.join(
                self.inference_model, "{}-G2.ckpt".format(self.submodel)
            )
            self.G2.load_state_dict(
                torch.load(G2_path, map_location=lambda storage, loc: storage)
            )
        ...
        if self.submodel == "NoTarget":
            ...
            drug_smiles = [
                line
                for line in open("DrugDesigner/data/akt_train.smi", "r")
                .read()
                .splitlines()
            ]
        ...
        self.inf_dataset = DruggenDataset(
            self.mol_data_dir,
            self.inf_dataset_file,
            self.inf_raw_file,
            self.max_atom,
        ...
        self.inf_drugs_loader = DataLoader(
            self.inf_drugs, shuffle=True, batch_size=self.inf_batch_size, drop_last=True
        )  # PyG dataloader for the second GAN.
        ...
        if not os.path.exists(
            "DrugDesigner/experiments/inference/{}".format(self.submodel)
        ):
            os.makedirs("DrugDesigner/experiments/inference/{}".format(self.submodel))
        with torch.inference_mode():

            dataloader_iterator = iter(self.inf_drugs_loader)
            pbar = tqdm(range(self.inference_sample_num))
            pbar.set_description(
                "Inference mode for {} model started".format(self.submodel)
            )
            for i, data in enumerate(self.inf_loader):
                ...
                inference_drugs = [
                    Chem.MolToSmiles(line) for line in fake_mol_g if line is not None
                ]
                inference_drugs = [
                    None if x is None else max(x.split("."), key=len)
                    for x in inference_drugs
                ]

                with open(
                    "DrugDesigner/experiments/inference/{}/inference_drugs.txt".format(
                        self.submodel
                    ),
                    "a",
                ) as f:
