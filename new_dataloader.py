...
class DruggenDataset(InMemoryDataset):
    ...
    def _generate_encoders_decoders(self, data):
        ...
        with open(
            "DrugDesigner/data/encoders/" + "atom_" + self.dataset_name + ".pkl", "wb"
        ) as atom_encoders:
            pickle.dump(self.atom_encoder_m, atom_encoders)

        with open(
            "DrugDesigner/data/decoders/" + "atom_" + self.dataset_name + ".pkl", "wb"
        ) as atom_decoders:
            pickle.dump(self.atom_decoder_m, atom_decoders)

        with open(
            "DrugDesigner/data/encoders/" + "bond_" + self.dataset_name + ".pkl", "wb"
        ) as bond_encoders:
            pickle.dump(self.bond_encoder_m, bond_encoders)

        with open(
            "DrugDesigner/data/decoders/" + "bond_" + self.dataset_name + ".pkl", "wb"
        ) as bond_decoders:
    ...
    def decoder_load(self, dictionary_name, file):
        with open(
            "DrugDesigner/data/decoders/" + dictionary_name + "_" + file + ".pkl", "rb"
        ) as f:
            return pickle.load(f)

    def drugs_decoder_load(self, dictionary_name):
        with open("DrugDesigner/data/decoders/" + dictionary_name + ".pkl", "rb") as f:
            return pickle.load(f)
    ...
    def matrices2mol(self, node_labels, edge_labels, strict=True, file_name=None):
        ...
        for start, end in zip(*np.nonzero(edge_labels)):
            if start > end:
                mol.AddBond(
                    int(start), int(end), bond_decoders[edge_labels[start, end]]
                )
    ...
    def process(self, size=None):
        ...
        for smiles in tqdm(
            smiles_list, desc="Processing chembl dataset", total=len(smiles_list)
        ):
            mol = Chem.MolFromSmiles(smiles)
            ...
            if A is not None:
                ...
                x = (
                    torch.from_numpy(self._genX(mol, max_length=max_length))
                    .to(torch.long)
                    .view(1, -1)
                )
                ...
                if self.features:
                    f = (
                        torch.from_numpy(self._genF(mol, max_length=max_length))
                        .to(torch.long)
                        .view(x.shape[0], -1)
                    )
                ...
                data = Data(
                    x=x, edge_index=edge_index, edge_attr=edge_attr, smiles=smiles
                )
        ...
        torch.save(
            self.collate(data_list), osp.join(self.processed_dir, self.dataset_file)
        )


if __name__ == "__main__":
    data = DruggenDataset("DrugDesigner/data")
